  ...
  <div class="viewport-height" :class="Store.user ? Store.theme : null">
    ...
    <div class="flex flex-col items-start fixed top-2 left-12 z-50" v-if="Store.messages">
      <FlashMessage v-for="message in Store.messages" :key="message.message" :message="message.message"
        :error="message.error" />
      <!-- <div class="bg-white mt-1 text-sm border-2  border-red-800 shadow-lg w-1/3 px-2 mx-2 rounded-lg text-np-base">Outage message here. Last updated: 14:09 UK</div> -->
    </div>
  </div>
</template>
