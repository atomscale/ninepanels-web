    ...
    <div>
      ...
      <div v-if="title.length > 0">
        <transition name="fade" appear>
          <DynamicButton class="mb-0.5" :parentMethod="sendCreatePanel" :buttonText="'Create panel'"
  ...
  methods: {
    async sendCreatePanel() {
      if (this.title.length === 0) {
        ...
        try {
          await this.panelStore.createPanelAction(this.emptySlotIndex, this.title, this.description)
