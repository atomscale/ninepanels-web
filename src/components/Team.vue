      ...
      <div class="mx-auto max-w-7xl py-12 px-4 sm:px-6 lg:px-8 lg:py-24">
        <div class="space-y-12">
          <div class="space-y-5 sm:space-y-4 md:max-w-xl lg:max-w-3xl xl:max-w-none">
            <h2 class="text-3xl font-bold tracking-tight text-gray-600 sm:text-4xl">The team</h2>
            <p class="text-xl text-gray-500">This product or servcie is made by a vast team of talent</p>
          </div>
          <ul role="list" class="space-y-4 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0 lg:grid-cols-3 lg:gap-8">
            <li v-for="person in people" :key="person.name" class="rounded-lg bg-gray-800 py-10 px-6 text-center xl:px-10 xl:text-left">
              <div class="space-y-6 xl:space-y-10">
                <img class="mx-auto h-40 w-40 rounded-full xl:h-56 xl:w-56 object-cover" :src="person.imageUrl" alt="" />
                <div class="space-y-2 xl:flex xl:items-center xl:justify-between">
                  <div class="space-y-1 text-lg font-medium">
                    <h3 class="text-white">{{ person.name }}</h3>
                    <p class="text-blue-400 -ml-10">{{ person.role }}</p>
                  </div>
